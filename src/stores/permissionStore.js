...
export const usePermissionStore = defineStore('permissionStore', {
  ...
  actions: {
    ...
    async create(formData) {
      try {
        this.loading = true
        const res = await fetch('/api/permissions', header('POST', this.token, formData))
        if (res.status === 500) {
          throw new Error(this.errorMessage)
        }
    ...
    async show(permission) {
      const res = await fetch(`/api/permissions/${permission}`, header('GET', this.token))
      const data = await res.json()
    ...
    async update(permission, formData) {
      try {
        this.loading = true
        const res = await fetch(`/api/permissions/${permission}`, header('PATCH', this.token, formData))
        if (res.status === 500) {
          throw new Error(this.errorMessage)
        }
    ...
    async deletePermission() {
      try {
        const res = await fetch(`/api/permissions/${this.permission}`, header('DELETE', this.token))
